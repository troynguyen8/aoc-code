...
function isRealRoom(roomCode) {
	var actualCheckSum = '';
	var letterCount = [];
	
	var letterArr = roomCode.match(/([a-z])/g);
	
	for(let i = letterArr.length - 5; i < letterArr.length; i++) {
		actualCheckSum = actualCheckSum + letterArr[i];
	}
	
	// deleting the check sum from the letter array
	letterArr.splice(letterArr.length - 5, 5);
	
	// this loop determines how often certain each letter occurs in the string and stores the letter / count in an object, letterSpecs
	for(let i = 0; i < letterArr.length; i++) {
		var currentCharCount = 0;
		var currentChar = letterArr[i];
		for(let j = 0; j < letterArr.length; j++) {
			if(currentChar === letterArr[j]) {
				currentCharCount++;
		...
		letterCount.push(letterSpecs);
		
		i = -1;
	}
	
	// This is the anonymous comparator function I was freaking out about
	// It sorts based on the passed in function! pretty wild
	...
	//After sorting, I take the first five letters and compare it to the five letters the code gave
...
for(let i = 0; i < input.length; i++) {
	if(isRealRoom(input[i])) {
		realRooms.push(input[i]);
	}
}

for(let i = 0; i < realRooms.length; i++) {
	var lastDashIndex = realRooms[i].lastIndexOf('-');
	var sectorID = parseInt(realRooms[i].match(/\d+/g))
	var shiftAmount = sectorID % 26;
	var adjustedString = '';
	
	for(let j = 0; j < lastDashIndex; j++) {
		if(realRooms[i].charAt(j) === '-') {
			adjustedString = adjustedString + ' ';
		} else {
			let currentChar = realRooms[i].charAt(j);
			
			for(let k = 0; k < alphabet.length; k++) {
				if(currentChar === alphabet.charAt(k)) {
					var currentCharValue = k;
				}
			}
			
			adjustedString = adjustedString + alphabet.charAt((currentCharValue + shiftAmount) % 26);
		}
	}
	
	if(adjustedString === 'northpole object storage') {
