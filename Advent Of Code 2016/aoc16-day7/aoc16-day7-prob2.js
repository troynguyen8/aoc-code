...
function supportsSSL(addressString) {
	if(addressString.search(/(?:\b\w+(?=\[))|(?:(\])\w+)/g) === -1) {
		return false;
	}

	if(addressString.search(/(?:\w+(?=\]))/g) === -1) {
		return false;
	}
	
	// lul do you see this regex expression. It's pretty wild
	var superNet = addressString.match(/(?:\b\w+(?=\[))|(?:(\])\w+)/g);
	var hyperNet = addressString.match(/(?:\w+(?=\]))/g);
	
	var abaMatches = [];
	for(let i = 0; i < superNet.length; i++) {
		for(let j = 0; j < superNet[i].length - 2; j++) {
			if(superNet[i].charAt(j) === superNet[i].charAt(j + 2) && superNet[i].charAt(j) !== superNet[i].charAt(j + 1)) {
				abaMatches.push(superNet[i].charAt(j) + superNet[i].charAt(j + 1) + superNet[i].charAt(j + 2));
			}
		}
	}
	
	
	if(abaMatches.length === 0) {
		return false;
	}
	
	for(let i = 0; i < abaMatches.length; i++) {
		var desiredString = abaMatches[i].charAt(1) + abaMatches[i].charAt(0) + abaMatches[i].charAt(1);
		
		for(let j = 0; j < hyperNet.length; j++) {
			if(hyperNet[j].includes(desiredString)) {
				return true;
			}
		}
	}
