 ...
 * @author Troy Nguyen
 * 12/20/16
 * Advent of Code 2016, Day 7 problem 1
 */
...
function supportsTLS(addressString) {
	if(addressString.search(/(\w)(\w)\2\1/g) === -1) {
		return false;
	}
	
	var abbaMatches = addressString.match(/(\w)(\w)\2\1/g);
	// TODO find a regex that includes the above regex but excludes /(\w)\1{3}/g
	for(let i = 0; i < abbaMatches.length; i++) {
		if(abbaMatches[i].search(/(\w)\1{3}/g) !== -1) {
			abbaMatches.splice(i, 1);
			i = -1;
		}
	}
	
	if(abbaMatches.length === 0) {
		return false;
	}
	
	var hyperNetSequences = addressString.match(/[\[]\w+[\]]/g);
	
	for(let i = 0; i < hyperNetSequences.length; i++) {
		for(let j = 0; j < abbaMatches.length; j++) {
			if(hyperNetSequences[i].includes(abbaMatches[j])) {
				return false;
			}
		}
	}
