 ...
 * @author Troy Nguyen 
 * 12/27/16
 * Advent of Code 2016, day 10 problem 1
 */
...
class Bot {
	...
	toString() {
		let botString = `BotID; ${this.id}\nChips:`;
		this.chips.forEach(chip => { botString = `${botString} ${chip}`});
...
input.forEach((inputString, index) => {
	if(inputString.includes('value')) {
		let givenValues = inputString.match(/\d+/g);
		let existingBot = false;
		for(let i = 0; i < botArr.length; i++) {
			if(botArr[i].id === givenValues[1]) {
				botArr[i].addChip(givenValues[0]);
				existingBot = true;
				break;
			}
		}
		...
		if(!existingBot) {
			botArr.push(new Bot(givenValues[1]));
			botArr[botArr.length - 1].addChip(givenValues[0]);
		}
...
for(let i = 0; i < botArr.length; i++) {
	if(botArr[i].hasBothChips()) {
		for(let j = 0; j < input.length; j++) {
			if(input[j].substring(0, 8).includes(`bot ${botArr[i].id}`) {
				if(input[j].search(/output \d+/) !== -1) {
					if(input[j].substring(8, input[j].length + 1).search(/bot \d+/) !== -1) {
...
console.log(input);
botArr.forEach(bot => console.log(bot.toString()));