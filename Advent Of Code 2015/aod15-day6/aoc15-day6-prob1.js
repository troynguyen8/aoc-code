...
// makes the matrix have 1000 rows
for(let i = 0; i < 1000; i++) {
	lightGrid.push([]);
}

// fills each row with 1000 falses
for(let i = 0; i < 1000; i++) {
	for(let j = 0; j < 1000; j++) {
		lightGrid[i].push(false);
	}
}

for(let i = 0; i < input.length; i++) {
	var coordRange = input[i].match(/\d+/g);
	
	for(let j = parseInt(coordRange[1]); j <= parseInt(coordRange[3]); j++) {
		for(let k = parseInt(coordRange[0]); k <= parseInt(coordRange[2]); k++) {
			if(input[i].includes('on')) {
				lightGrid[j][k] = true;
			} else if(input[i].includes('off')) {
				lightGrid[j][k] = false;
			} else {
				lightGrid[j][k] = !(lightGrid[j][k]);
			}
		}
	}
}
