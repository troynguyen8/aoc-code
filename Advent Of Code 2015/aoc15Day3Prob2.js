 ...
 * This program displays how many houses receive at least one present given a string of text with directions
 */
...
// var directions = '^>v<';
// var directions = '^v^v^v^v^v';

function isEqualHouse(house1, house2) {
	if(house1.xCoord === house2.xCoord && house1.yCoord === house2.yCoord) {
		return true;
	}
...
var currentSantaXCoord = 0;
var currentSantaYCoord = 0;
var currentRoboXCoord = 0;
var currentRoboYCoord = 0;
var houseArray = [];
...
for(var i = 0; i < directions.length; i++) {
	if(i % 2 === 0) {
		...
		house = {
		xCoord: currentSantaXCoord,
		yCoord: currentSantaYCoord
		};
	} else {
		if(directions.charAt(i) === 'v') {
			currentRoboYCoord -= 1;
		...
		house = {
			xCoord: currentRoboXCoord,
			yCoord: currentRoboYCoord
		};
	}
	
	
	for(var j = 0; j < houseArray.length; j++) {
		var isUniqueHouse = true;
		if(isEqualHouse(house, houseArray[j])) {
			isUniqueHouse = false;
			break;
		}
	}
