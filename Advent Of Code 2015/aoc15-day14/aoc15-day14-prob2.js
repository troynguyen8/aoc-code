 ...
 * @author Troy Nguyen
 * 12/28/16
 * Advent of Code 2015, Day 14 problem 2
 */
...
for(let totalTime = 1; totalTime <= 2503; totalTime++) {
	...
	let maxDist = reindeerArr[0].distanceTraveled;
	reindeerArr.forEach(reindeer => maxDist = Math.max(maxDist, reindeer.distanceTraveled));
	
	reindeerArr.forEach(reindeer => {
		if(reindeer.distanceTraveled === maxDist) {
			reindeer.points++
		}
	});
}

let maxPoints = reindeerArr[0].points;
reindeerArr.forEach(reindeer => maxPoints = Math.max(maxPoints, reindeer.points));
