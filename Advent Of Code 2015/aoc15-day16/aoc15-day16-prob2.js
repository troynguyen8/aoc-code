 ...
 * @author Troy Nguyen
 * 12/31/16
 * Advent of Code 2015, Day 16 problem 2
 */
...
function isFakeSue(parameter, index) {
	if(typeof sues[index][parameter] === 'undefined' || sues[index][parameter] === 0) {
		return false;
	}
	
	if(parameter === 'pomeranians' || parameter === 'goldfish') {
		return (sues[index][parameter] >= knownBelongings[parameter]);
	} else if(parameter === 'cats' || parameter === 'trees') {
		return (sues[index][parameter] <= knownBelongings[parameter]);
	} else {
		return (sues[index][parameter] !== knownBelongings[parameter]);
	}
